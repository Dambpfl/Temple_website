...
const linkAction = () =>{
    const navMenu = document.getElementById('nav-menu')
    // Quand tu clique sur n'imp quel lien du menu, le ferme
    navMenu.classList.remove('show-menu')
}
...
const blurHeader = () =>{
    ...
    // Ajoute une classe si le décalage inférieur est supérieur à 50 de la fenêtre d’affichage (MENU FLOU)
...
/*=============== GSAP ANIMATION ===============*/
gsap.from('.home__img-1', 5, {opacity: 0, x: 400}) /* IMG + TEMPS + LIEU DE DEPART (DELAIS + SAUT(HAUTEUR)) */
gsap.from('.home__img-6', 1.5, {opacity: 0, y: 200, delay: .1, ease: 'back.out(1.5)'})
gsap.from('.home__img-3', 1.5, {opacity: 0, y: 400, delay: .1, ease: 'back.out(1.5)'})
...
gsap.from('.home__data', 1.5, {opacity: 0, y: -100, delay: 2.5, ease: 'back.out(1.5)'}) /* DATA ICI*/
